...
const Modal = {
  ...
  render_input: function (type) {
    ...
    for (let key in attributes) {
      ...
      let input_group_template = $('#attribute-reason-input-group-template').html();
  ...
  save: function () {
    ...
    for (let i = 1; i <= this.state.max_attribute; i++) {
      ...
      const reason1 = $('#' + attribute_id + '-reason1-val').html().trim().split(' ').join('_');
      const reason2 = $('#' + attribute_id + '-reason2-val').html().trim().split(' ').join('_');

      if (!reason1 || !reason2) {
        toastr.warning("Please annotate on all attributes.", '', {progressBar: false});
        return false;
      }
  ...
  input_listen: function () {
    ...
    $('.dropdown-item').click(function () {
      ...
      Modal.state.step = Number(attribute_id.replace('attribute', '').replace('-val', '').replace('-reason2', '').replace('-reason1', ''));
      ...
      if (attribute_id.indexOf('reason') < 0) {
        ...
        $('.input-group-text').removeClass('text-primary');
        $('#attribute' + Modal.state.step + '-reason1').parents('.input-group').find('.input-group-text').addClass('text-primary');
        $('.dropdown-toggle').removeClass('text-primary');
        $('#attribute' + Modal.state.step + '-reason1-val').addClass('text-primary');
        ...
        const option_value = $('#' + attribute_id + '-val').html().trim();
        ...
        $('.input-group-text').removeClass('text-primary');
        $('#attribute' + Modal.state.step + '-reason2').parents('.input-group').find('.input-group-text').addClass('text-primary');
        $('.dropdown-toggle').removeClass('text-primary');
        $('#attribute' + Modal.state.step + '-reason2-val').addClass('text-primary');
