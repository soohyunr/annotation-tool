...
## Citation
This tool is a toy version annotation tool that we built before the development of the professional annotation tool that was used for the "CredOn" annotation project, which has some extra functionalities for the annotation monitoring. Even though it is a toy version tool, except for the extra functionalities such as the monitoring and the different annotation scheme for local acceptability, the basic functionalities are the same :) The annotation project is introduced in the following paper. When you use this tool, please cite the paper:
